...
function buildTempoRegion( song: Song, length: number, lowestY: number, options: GenerateOptions ): LitematicRegion {
...
function paletteColumns(
  ...
  columns.sort(
    ( a, b ) => Number( a.tiers.length > 1 ) - Number( b.tiers.length > 1 ) || ( a.soundId < b.soundId ? -1 : 1 )
  )
  return columns
}
...
export function generateLitematic(
  ...
  if ( options.paletteRegion !== "none" ) {
    const palette = buildPaletteRegion( paletteColumns( song, orderedLayerIds, allocation, options.paletteRegion ) )
