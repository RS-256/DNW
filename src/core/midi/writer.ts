...
export function writeMidi( song: Song, layers: Layer[], options: MidiOptions ): MidiResult {
  ...
  if ( denominatorRounds > 0 ) {
    warnings.push( `${ denominatorRounds } time-signature denominator(s) are not powers of two and were rounded.` )
  }
  ...
  if ( panConflict ) {
    warnings.push( "Tracks with different pans share a MIDI channel (same instrument); the last pan wins." )
  }
  if ( centsDropped > 0 ) {
  ...
  const out = new Uint8Array( header.length + trackChunks.reduce( ( sum, chunk ) => sum + chunk.length, 0 ) )
  out.set( header )
