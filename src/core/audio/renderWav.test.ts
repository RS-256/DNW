...
describe( "encodeWav", () => {
  ...
  it( "writes a valid RIFF header for 16-bit stereo", () => {
    const wav = encodeWav( stubPcm( [ new Float32Array( [ 0, 0.5 ] ), new Float32Array( [ 1, -1 ] ) ], 44100 ), int16 )
  ...
  it( "interleaves channels and maps [-1, 1] to int16 with clipping", () => {
    const wav = encodeWav( stubPcm( [ new Float32Array( [ 0, 1, 2 ] ), new Float32Array( [ 0.5, -1, -3 ] ) ] ), int16 )
    const view = new DataView( wav )
  ...
  it( "rejects float at depths below 32-bit", () => {
    expect( () => encodeWav( stubPcm( [ new Float32Array( [ 0 ] ) ] ), { bitDepth: 16, float: true } ) ).toThrow()
  } )
} )
...
describe( "flattenForRender", () => {
  it( "bakes velocity, instrument, layer and group gains in and clamps pan", () => {
    ...
    layer.volume = 80
    layer.pan = 0.5
    layer.notes = [ createNote( { tick: 20, instrument: 0, key: 45, velocity: 100, pan: -1, pitch: 0 } ) ]
