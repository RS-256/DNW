 ...
 */
import { createLayer, createNote, createVanillaInstrument, DEFAULT_PITCH_KEY, newId } from "../model/song"
...
export function readNbs( buffer: ArrayBuffer ): Song {
  ...
  const finalInstruments = instruments.filter( ( inst ) => inst.isVanilla || inst.name !== TEMPO_CHANGER_NAME )
