...
export default function MidiImportModal( { fileName, midi, onImport, onCancel }: MidiImportModalProps ) {
  const [ raw, setRaw ] = useState( "8" )
  const value = /^\d+$/.test( raw.trim() ) ? Number( raw.trim() ) : NaN
  const valid = Number.isInteger( value ) && value >= 1
  const analysis = useMemo( () => analyzeMidiTiming( midi, valid ? value : 1 ), [ midi, valid, value ] )
      ...
      <div className="dialog midi-dialog" onClick={ ( e ) => e.stopPropagation() }>
        ...
        { lossy && (
          <p className="midi-warning">
            ...
            { gridNoteValue( value ) }-note grid of gt/quarter = { value } and will be snapped to the nearest game tick.
          </p>
        ) }
        { lossy && analysis.losslessTickPerQuarter !== null && (
          <p className="midi-warning">
            Lossless import needs gt/quarter = { analysis.losslessTickPerQuarter } (or a multiple of it).
          </p>
        ) }
        { lossy && analysis.losslessTickPerQuarter === null && (
          <p className="midi-warning">
            No practical gt/quarter fits this file exactly — its timing is probably humanized (not quantized).
          </p>
        ) }
        { valid && ! lossy && (
          <p className="midi-note">
            { analysis.losslessTickPerQuarter !== null && analysis.losslessTickPerQuarter < value ? (
              <>
                Every event fits this grid exactly (gt/quarter = { analysis.losslessTickPerQuarter } would already be
                enough).
              </>
            ) : (
        ...
        <p className="midi-note">
          Note blocks cannot sustain, so MIDI note lengths are ignored: each note becomes a single one-tick note at its
          start position. Tempo and time-signature changes are imported into the tempo track.
        </p>
        <div className="confirm-buttons">
          <button className="confirm-cancel" onClick={ onCancel }>
            Cancel
          </button>
          <button className="export-confirm" disabled={ ! valid } onClick={ () => valid && onImport( value ) }>
            Import
          </button>
        </div>
      </div>
