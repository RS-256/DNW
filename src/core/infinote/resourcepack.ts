...
export function collectManagedInstruments(
  song: Song,
  ...
  return song.instruments.filter(
    (inst, i) => used.has(i) && !inst.isVanilla && !!inst.soundSourceId,
  );
}
 ...
 */
export async function detectStereoSounds(
  sounds: { name: string; data: ArrayBuffer }[],
): Promise<string[]> {
