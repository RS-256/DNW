...
export default function PianoRoll() {
  ...
  useEffect( () => {
    ...
    const drawLane = () => {
      markersRef.current = renderTempoLane( lane.getContext( "2d" )!, vp, useSongStore.getState().song )
    }
    ...
    const drawOverlay = () => {
      ...
      const state: OverlayState = {
        playheadTick: playing ? audioEngine.currentTick() : usePlaybackStore.getState().positionTick,
        playheadActive: playing,
        selectionBox: drag.type === "box" ? { x0: drag.startX, y0: drag.startY, x1: drag.curX, y1: drag.curY } : null,
        ghost:
          drag.type === "move" && drag.moved
            ? {
                ...
                dTick: drag.dTick,
                dKey: drag.dKey,
                color: useSongStore.getState().song.layers[ useEditorStore.getState().activeLayer ]?.color ?? "#ffffff"
              }
            : null
      }
    ...
    const onMouseDown = ( e: MouseEvent ) => {
      ...
      if ( e.button === 2 ) {
        dragRef.current = {
          ...
          curX: px.x,
          curY: px.y,
          base: e.ctrlKey || e.metaKey ? new Set( useEditorStore.getState().selection ) : undefined
        }
        drawOverlay()
        return
      }
  ...
  return (
    <div ref={ containerRef } className="pianoroll" tabIndex={ -1 }>
      ...
      { tempoEditor && <TempoEventEditor target={ tempoEditor } onClose={ () => setTempoEditor( null ) } /> }
    </div>
  )
}
