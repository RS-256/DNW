...
export default function TempoEventEditor( { target, onClose }: { target: TempoEditorTarget; onClose: () => void } ) {
  ...
  const [ numerator, setNumerator ] = useState( editing?.type === "timeSignature" ? editing.numerator : 4 )
  const [ denominator, setDenominator ] = useState( editing?.type === "timeSignature" ? editing.denominator : 4 )
    ...
    <div className="tempo-editor" style={ { left: target.x, top: target.y } }>
      ...
      { ! editing && (
        <div className="tempo-editor-row">
          ...
          <label>
            <input type="radio" checked={ type === "timeSignature" } onChange={ () => setType( "timeSignature" ) } />{ " " }
            time sig
          </label>
        </div>
      ) }
