...
export function placeTrack(
  ...
  for (const raw of layer.notes) {
    ...
    if (!baseBlock) {
      throw new Error(
        `No base block assigned for slot ${slotKey(effectiveSoundId(inst), sound.pitchShift)}`,
      );
    }
    ...
    outer: for (let delta = 0; delta <= MAX_NUDGE && dz === null; delta++) {
      ...
      magnitudes.sort(
        (a, b) =>
          dbError(targetGain, gain(Math.hypot(depth, a))) -
...
export function placeSong(
  ...
  if (overThreshold > 0) {
    warnings.push(
      `${overThreshold} note(s) exceed 1 dB placement error (max ${maxDbError.toFixed(2)} dB)`,
    );
  }
