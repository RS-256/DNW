...
describe( "readMidi", () => {
  it( "reads notes, tempo, time signature, track names and running status", () => {
    const track0 = [
      ...
      0x03,
      0x24,
      0x08, // time signature 3/8
      ...vlq( 0 ),
      0x90,
      0x45,
      0x64, // note-on A4 (69) vel 100
      ...vlq( 240 ),
      0x45,
      0x00, // running status, vel 0 = note-off → dropped
      ...vlq( 0 ),
      0x40,
      0x50 // running status note-on E4 (64) vel 80
    ]
    const track1 = [
      ...vlq( 0 ),
      0x99,
      0x26,
      0x7f // percussion channel, note 38, vel 127
    ]
...
describe( "writeMidi", () => {
  it( "roundtrips header, names, tempo map and notes through readMidi", () => {
    const song = songWithNotes( [ { tick: 2, instrument: HARP, key: 45, velocity: 100, pan: 0, pitch: 0 } ] )
  ...
  it( "writes program, channel volume and pan at track start", () => {
    const song = songWithNotes( [ { tick: 0, instrument: HARP, key: 45, velocity: 50, pan: 0, pitch: 0 } ] )
  ...
  it( "applies the vanilla transpose table and midiProgram overrides", () => {
    const song = songWithNotes( [ { tick: 0, instrument: BASS, key: 45, velocity: 100, pan: 0, pitch: 0 } ] )
  ...
  it( "sends percussion to channel 10 with fixed keys and no program change", () => {
    const song = songWithNotes( [ { tick: 0, instrument: BASEDRUM, key: 50, velocity: 100, pan: 0, pitch: 0 } ] )
