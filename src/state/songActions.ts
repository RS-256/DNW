...
import type { BpmEvent, Instrument, Layer, Note, TempoEvent, TrackGroup } from "../core/model/types"
...
export function updateInstrument(
  index: number,
  props: Partial< Pick< Instrument, "name" | "pitchKey" | "volume" | "soundId" | "baseBlock" | "midiProgram" > >
): void {
  useSongStore.getState().mutate( ( draft ) => {
...
export function setInitialBpm( bpm: number ): void {
  ...
  useSongStore.getState().mutate( ( draft ) => {
    const ev = draft.tempoTrack.events.find( ( e ): e is BpmEvent => e.type === "bpm" && e.tick === 0 )
    if ( ev ) ev.bpm = bpm
  } )
}
